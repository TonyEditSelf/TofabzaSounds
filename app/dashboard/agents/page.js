...
/**
 * app/dashboard/agents/page.js
 *
 * Lists all agents. Filtered by activeClientId from Zustand.
 * Click row → /dashboard/agents/[id]
...
async function fetchAgents(clientId) {
  let q = supabase
    .from("agents")
    .select(
      "id, name, type, status, language, config, created_at, clients(name)",
    )
  ...
  return (
    <div>
      <div style={s.pageHeader}>
        <h1 style={s.pageTitle}>Agents</h1>
        <button
            ...
            {isLoading ? (
              ...
              agents.map((a) => (
                ...
                >
                  ...
                  <td style={s.td}>{a.clients?.name ?? "—"}</td>
                  <td style={s.td}>
                    <span
                      style={{
                        fontFamily: "var(--font-mono)",
                  ...
                  <td style={s.td}>
                    <span
                      style={{
                        fontSize: "0.75rem",
                        fontWeight: 500,
                        color: a.status === "active" ? "#16A34A" : "#9CA3AF",
                      }}
                    >
                      {a.status}
                    </span>
