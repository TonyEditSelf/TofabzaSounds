...
const DEPRECATED_SETTING_KEYS = new Set(["gemini_api_key"]);
 ...
 * Falls back to process.env for any key not found in DB.
 */
export async function getSettings() {
  ...
  for (const row of data ?? []) {
    if (DEPRECATED_SETTING_KEYS.has(row.key)) continue;
    try {
...
function buildEnvFallback() {
  return {
    voice_provider: process.env.VOICE_PROVIDER ?? "sarvam",
    // Sarvam (telephony pipeline only — STT/TTS for phone calls)
    sarvam_api_key: process.env.SARVAM_API_KEY ?? "",
    sarvam_default_language: process.env.SARVAM_DEFAULT_LANGUAGE ?? "ml-IN",
    sarvam_default_voice: process.env.SARVAM_DEFAULT_VOICE ?? "anand",

    // Google Cloud (TTS + STT)
    google_tts_api_key: process.env.GOOGLE_TTS_API_KEY ?? "",
    google_stt_api_key: process.env.GOOGLE_STT_API_KEY ?? "",
    google_default_language: process.env.GOOGLE_DEFAULT_LANGUAGE ?? "ml-IN",
    google_default_voice: process.env.GOOGLE_DEFAULT_VOICE ?? "ml-IN-Wavenet-B",
    google_tts_model: process.env.GOOGLE_TTS_MODEL ?? "wavenet", // wavenet | neural2
