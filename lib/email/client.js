...
/** Operator email â€” the only recipient for all system emails */
const OPERATOR_EMAIL = process.env.OPERATOR_EMAIL;

/** From address â€” must be a verified domain in Resend */
...
export async function sendOperatorEmail({ subject, html, text }) {
  if (!OPERATOR_EMAIL) {
    console.error("[email] OPERATOR_EMAIL not set â€” cannot send email");
    return null;
  }
...
// =============================================================
// EMAIL TEMPLATES
// Simple HTML strings â€” no JSX dependency needed for Phase 0.
...
export async function sendLowCostAlert({
  ...
  const subject = `âš ï¸ Cost alert: ${clientName} has exceeded â‚¹${thresholdInr.toLocaleString("en-IN")}`;
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
      ...
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
        ...
        <tr>
          ...
          <td style="padding: 12px; border: 1px solid #E2E4EF; font-weight: 600; color: #E11D48;">
            â‚¹${costInr.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
          </td>
        </tr>
        <tr>
          <td style="padding: 12px; border: 1px solid #E2E4EF; color: #4A4E6B;">Alert threshold</td>
          <td style="padding: 12px; border: 1px solid #E2E4EF;">â‚¹${thresholdInr.toLocaleString("en-IN")}</td>
        </tr>
      </table>
      ...
      <p style="color: #7C82A3; font-size: 12px; margin-top: 24px;">
        Sarvam Agency Platform Â· Automated alert
      </p>
    </div>
  `;
...
export async function sendAgentErrorAlert({
  ...
  const subject = `ðŸ”´ Circuit breaker open: ${service} â€” ${agentName} affected`;
...
export async function sendNdncFailureAlert({
  ...
  const subject = `âš ï¸ NDNC check failures: ${campaignName} â€” ${Math.round(failureRate * 100)}% failed`;
...
export async function sendCampaignCompletedEmail({
  ...
  const subject = `âœ… Campaign complete: ${campaignName}`;
  const html = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;">
      ...
      <p style="color:#7C82A3;font-size:12px;margin-top:24px;">Tofabza Sounds Â· Automated alert</p>
    </div>
  `;
  return sendOperatorEmail({ subject, html });
}
