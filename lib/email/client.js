 ...
 * @returns {Promise<{ id: string }|null>}
 */
export async function sendLowCostAlert({
  clientName,
  costInr,
  thresholdInr,
  dashboardUrl,
...
export async function sendCampaignCompletedEmail({
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;">
      <h2 style="color:#0A0B0F;margin-bottom:8px;">Campaign Completed</h2>
      <p style="color:#4A4E6B;margin-bottom:24px;"><strong>${campaignName}</strong> has finished running.</p>
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
        <tr>
          <td style="padding:12px;border:1px solid #E2E4EF;color:#4A4E6B;">Total contacts</td>
          <td style="padding:12px;border:1px solid #E2E4EF;font-weight:600;">${total}</td>
        </tr>
        <tr>
          <td style="padding:12px;border:1px solid #E2E4EF;color:#4A4E6B;">Answered</td>
          <td style="padding:12px;border:1px solid #E2E4EF;font-weight:600;color:#16A34A;">${answered}</td>
        </tr>
        <tr>
          <td style="padding:12px;border:1px solid #E2E4EF;color:#4A4E6B;">Failed / No answer</td>
          <td style="padding:12px;border:1px solid #E2E4EF;font-weight:600;color:#E11D48;">${failed}</td>
        </tr>
      </table>
      <a href="${dashboardUrl}/dashboard/campaigns"
         style="display:inline-block;background:#0A0B0F;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:500;font-size:14px;">
        View Campaign
      </a>
      <p style="color:#7C82A3;font-size:12px;margin-top:24px;">Tofabza Sounds · Automated alert</p>
    </div>
  `;
  return sendOperatorEmail({ subject, html });
}
...
/**
 * Unknown domain alert — widget accessed from unauthorised origin.
 * @param {object} params
 * @param {string} params.widgetName
 * @param {string} params.origin - the unauthorised origin
 * @param {string} params.dashboardUrl
 */
export async function sendUnknownDomainEmail({
  widgetName,
  origin,
  dashboardUrl,
}) {
  const subject = `⚠️ Unauthorised widget access: ${origin}`;
  const html = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;">
      <h2 style="color:#D97706;margin-bottom:8px;">Unauthorised Domain Detected</h2>
      <p style="color:#4A4E6B;margin-bottom:24px;">
        Someone tried to load the widget <strong>${widgetName}</strong> from an origin not on your allowed list.
      </p>
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
        <tr>
          <td style="padding:12px;border:1px solid #E2E4EF;color:#4A4E6B;">Widget</td>
          <td style="padding:12px;border:1px solid #E2E4EF;font-weight:600;">${widgetName}</td>
        </tr>
        <tr>
          <td style="padding:12px;border:1px solid #E2E4EF;color:#4A4E6B;">Unauthorised origin</td>
          <td style="padding:12px;border:1px solid #E2E4EF;font-family:monospace;color:#E11D48;">${origin}</td>
        </tr>
      </table>
      <a href="${dashboardUrl}/dashboard/widgets"
         style="display:inline-block;background:#0A0B0F;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:500;font-size:14px;">
        Manage Widgets
      </a>
      <p style="color:#7C82A3;font-size:12px;margin-top:24px;">Tofabza Sounds · Automated alert</p>
